...
const popupCancelBuyNftViewComponent = {
  ...
  <div class="fix-m-4" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-primary btn-sm" type="button" v-on:click="onModifyBuyNftClick()">구매입찰 수정하기</button>
        <button class="btn btn-primary btn-sm" type="button" v-on:click="onCancelBuyNftClick()">구매입찰 취소하기</button>
      </div>
    </div>
  </div>
  ...
  , methods : {
    ...
    onModifyBuyNftClick : function () {
      if (this.agreement != true) {
        utility.errorAlert("", "주의사항을 읽고 동의를 해주세요");
        return;
      }
      this.isLoading = true;
      assetsProvider.modifyBuyAsset(this, function ($this, result) {
        $this.isLoading = false;
        if (result.code == "C0000") {
          utility.successAlert("", "구매입찰 수정 성공", function () {
    ...
    onCancelBuyNftClick : function() {
      ...
      assetsProvider.cancelBuyAsset(this, function ($this, result) {
        if (result.code == "C0000") {
          utility.successAlert("", "구매입찰 취소 성공", function () {
            window.location.reload();
          });
        } else {
          utility.errorCodeAlert("구매입찰 취소 실패", result.code, result.msg, function () {
            window.location.reload();
          });
        }
      }, this.asset.id);
